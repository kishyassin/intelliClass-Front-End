...
class IntelliClassSupabaseUser extends BaseAuthUser {
  IntelliClassSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> intelliClassSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = IntelliClassSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
